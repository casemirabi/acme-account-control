        ...
        try {
            ...
            if (started.status === 'completed') {
                ...
                if (dadosRaw) {
                    ...
                    const msg =
                        (started.error && started.error.message) ? started.error.message :
                            (started.message) ? started.message :
                                'Consulta finalizada, porém sem dados para exibir.';
                    renderMsg(box, msg, 'err'); // ou 'ok' se quiser tratar como info
        ...
        */
        if (saved && saved.status === 'completed') {
            if (saved.dados) {
                ...
                renderMsg(box, saved.error || 'Consulta finalizada, porém sem dados para exibir.', 'err');
            }
            clearLastResult();
        } else if (saved && saved.status === 'failed') {
            renderMsg(box, saved.error || 'Falha na consulta.', 'err');
