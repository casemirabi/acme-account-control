 ...
 * - Tabelas helpers: acme_table_credit_transactions(), acme_table_services()
 * - Helpers de escopo: acme_get_grandchildren_of_child() (opcional)
 * - (Opcional para PDF bonito) helpers.php:
 *   - acme_load_dompdf()
...
/**
 * ============================================================
 * UI (mesmo padrão do acme_clt_panel) — CSS injetado 1x
 * ============================================================
 */
...
.acme-btn{display:inline-block;padding:8px 12px;border-radius:10px;border:1px solid #e2e8f0;background:#f8fafc;text-decoration:none;font-weight:800;color:#0f172a;cursor:pointer}
...
/* Wrapper pra permitir scroll horizontal se o Elementor deixar estreito */
.acme-fields-wrap{
  overflow-x:auto;
...
/* ✅ 5 campos lado a lado SEMPRE (igual ao teu print) */
.acme-fields{
...
/* evita quebra/estouro */
.acme-fields > div{min-width:0 !important;}
.acme-fields label{display:block;font-size:12px;margin-bottom:6px;color:#64748b;font-weight:800}
...
.acme-actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:10px 0 2px 0}
...
add_action('admin_post_acme_export_credit_transactions', function () {
  ...
  error_log('ACME dompdf check: ' . (function_exists('acme_pdf_is_available') ? (acme_pdf_is_available() ? 'YES' : 'NO') : 'missing func'));
  error_log('ACME_ACC_PATH: ' . (defined('ACME_ACC_PATH') ? ACME_ACC_PATH : 'not defined'));
...
if (!function_exists('acme_render_transactions_table')) {
  function acme_render_transactions_table(string $context = 'front'): string
  {
    ...
    $user_id   = isset($_GET['user_id']) ? (int) $_GET['user_id'] : 0;
    $service_id = isset($_GET['service_id']) ? (int) $_GET['service_id'] : 0;
    $date_from = isset($_GET['from']) ? sanitize_text_field((string) $_GET['from']) : ''; // YYYY-MM-DD
    $date_to   = isset($_GET['to']) ? sanitize_text_field((string) $_GET['to']) : '';     // YYYY-MM-DD
    ...
    // Links de export mantendo os filtros atuais (RBAC é aplicado no handler)
    $export_args = $_GET;
    ...
    $export_args['format'] = 'pdf';
    $pdf_url = add_query_arg($export_args, admin_url('admin-post.php'));

    // limpar filtros (para botão no header)
    $has_any_filter =
      //($q !== '') || 
      ($type !== '')
      || ($status !== '')
...
?>
    <style>
      /* Mesmo padrão do painel anterior */
      .acme-actions {
        display: flex;
        gap: 8px;
        flex-wrap: wrap;
      ...
      /* Linha 3: Filtrar + Excel (e opcional PDF) */
      .acme-filter-row-actions {
        display: flex;
        gap: 10px;
        flex-wrap: wrap;
        align-items: center;
    ...
    <div class="acme-panel">
        ...
        <div class="acme-actions">
          ...
          <?php if ($has_any_filter): ?>
            <a class="acme-btn" style="background:#fff;color:#0f172a;border:1px solid #e2e8f0;"
              href="<?php echo esc_url(remove_query_arg(['q', 'type', 'status', 'user_id', 'service_id', 'from', 'to', 'paged'])); ?>">
              Limpar filtros
            </a>
          <?php endif; ?>
        </div>
      ...
      <div style="padding:14px 16px;">
        ...
        <form method="get" class="acme-filter-grid">
          ...
          <!-- Ações: Filtrar + Export -->
          <div class="acme-filter-row-actions">
            <button type="submit" class="acme-btn">Filtrar</button>

            <a class="acme-btn" href="<?php echo esc_url($xls_url); ?>" title="Baixar Excel">📊 Excel</a>
            <!-- Se quiser habilitar o PDF depois, é só descomentar:
            <a class="acme-btn" href="<?php echo esc_url($pdf_url); ?>" title="Baixar PDF">📄 PDF</a>
            -->
          </div>

        </form>

        <p class="acme-muted" style="margin:12px 0 0 0">
      ...
      <?php if (empty($rows)): ?>
        <div style="padding:14px 16px;color:#64748b;">Nenhuma transação encontrada.</div>
    </div>
      ...
      <tbody>
        <?php foreach ($rows as $r): ?>
          <tr>
            ...
            <!-- <td class="acme-muted"><?php #echo (int) $r->id; 
                                        ?></td>-->
            <!--<td class="acme-muted"><?php #echo esc_html($r->created_at); 
                                        ?></td>-->
            <td class="acme-muted">
              <?php echo esc_html(date('d/m/Y H:i:s', strtotime($r->created_at))); ?>
            </td>
            ...
            <!--<td class="acme-mono"><?php #echo esc_html($r->type); 
                                      ?></td>-->
            <td class="acme-mono">
              <?php
              echo esc_html(
                $r->type === 'credit' ? 'Creditado' : ($r->type === 'debit' ? 'Debitado' : $r->type)
              );
              ?>
            </td>
            ...
            <td class="acme-mono">
              <?php
              echo esc_html(
                $r->status === 'success' ? 'Sucesso' : ($r->type === 'failed' ? 'Falha' : $r->status)
              );
              ?>
            </td>
  ...
  ?>
    <div style="padding:12px 16px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;border-top:1px solid #f1f5f9">
      <?php for ($p = 1; $p <= $total_pages; $p++):
        $url = add_query_arg('paged', $p, $base);
        $active = ($p === $page);
      ?>
        <a href="<?php echo esc_url($url); ?>" class="acme-btn"
          style="<?php echo $active ? 'background:#111827;color:#fff;border-color:#111827' : ''; ?>">
          <?php echo (int) $p; ?>
        </a>
      <?php endfor; ?>
    </div>
