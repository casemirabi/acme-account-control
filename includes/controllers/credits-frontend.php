...
/**
 * Handler: Recuperar créditos (Admin -> Master) e (Master -> Sub-Login)
 * action: acme_recover_credits
 */
if (!function_exists('acme_handle_recover_credits')) {
  add_action('admin_post_acme_recover_credits', 'acme_handle_recover_credits');

  function acme_handle_recover_credits() {
    if (!is_user_logged_in()) {
      wp_die('Você precisa estar logado.');
    }
    ...
    if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'acme_recover_credits')) {
      wp_die('Nonce inválido.');
    }

    $user_id = isset($_POST['user_id']) ? (int) $_POST['user_id'] : 0; // alvo (Master ou Sub-Login)
    $service_slug = isset($_POST['service_slug']) ? sanitize_key($_POST['service_slug']) : '';
    $credits = isset($_POST['credits']) ? (int) $_POST['credits'] : 0;
    $notes = !empty($_POST['notes']) ? sanitize_text_field($_POST['notes']) : null;
    ...
    if ($is_admin) {
      // Admin só recupera de Masters (role child)
      $target = get_user_by('id', $user_id);
      $is_target_child = $target && in_array('child', (array) $target->roles, true);

      if (!$is_target_child) {
        $res = new WP_Error('acme_target', 'Admin só pode recuperar de Masters.');
      } elseif (!function_exists('acme_lots_admin_recover_from_child')) {
        $res = new WP_Error('acme_missing', 'Função de recuperação não encontrada.');
      } else {
        $res = acme_lots_admin_recover_from_child($user_id, $service_id, $credits, $notes, $meta);
      }

    } else {
      // Master recupera de Sub-Login (grandchild)
      if (!function_exists('acme_lots_recover_grandchild_to_child')) {
        $res = new WP_Error('acme_missing', 'Função de recuperação não encontrada.');
      } else {
        $res = acme_lots_recover_grandchild_to_child((int)$me->ID, $user_id, $service_id, $credits, $notes, $meta);
      }
    }
    ...
    if (is_wp_error($res)) {
      ...
      $ok = (bool) $res['success'];
      if (!$ok && !empty($res['message'])) $err_msg = (string) $res['message'];
    } else {
      $ok = ($res === true);
    }
